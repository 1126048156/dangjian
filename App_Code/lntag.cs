...
public class lntag
{
            ...
            if (str2.ToString().Substring(startIndex, index - startIndex).ToString() != null)
            {
                ...
                try
                {
                    switch (str3)
                    {
                        ...
                        case "LnLNLT":
                            {
                                builder = builder.Replace(str2.Trim(), lnlab.LnNLT(System.Web.HttpContext.Current.Request.RequestContext.RouteData.Values["ID"].ToString(), tagID[1], tagID[2], tagID[3], tagID[4], tagID[5], tagID[6], tagID[7]));
                                continue;
                            }
    ...
    public string ReplaceHtmlTag(string temp)
    {
        ...
        SysSet sysset = new SysSet().loadCacheConfig(Utils.GetMapPath("~/Content/WebSet.config"));
    ...
    public string GetStaticTag(string htmls)
    {
       ...
       // htmls = htmls.Replace("{$Ln_head$}", GetWebContent("http://www.ynicity.cn/ecity-web/pages/common/head.jsp"));

        // htmls = htmls.Replace("{$Ln_foot$}", getURL("http://www.ynicity.cn/ecity-web/pages/common/foot.jsp"));
